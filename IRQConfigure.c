/**
 * roboRIO FPGA IRQ configuration wrapper source file
 * Copyright (c) 2015,
...
/**
 * Include the roboRIO header file.
...
/**
 * Declare the roboRIO NiFpga_Session so that it can be used by any function in
 * this file. The variable is actually defined in roboRIO.c.
 *
 * This removes the need to pass the roborio_session around to every function and
 * only has to be declared when it is being used.
 */
extern NiFpga_Session roborio_session;
...
int32_t Irq_CheckReserved(Irq_Channel channel, uint8_t irqNumber)
{
    ...
    if (pCurr != NULL)
    {
        do
        {
            ...
            {
                return NiRoborio_Status_IrqNumberNotUsable;
            }
            else if (channel == pCurr->channel)
            {
                return NiRoborio_Status_IrqChannelNotUsable;
            }
            pCurr = pCurr->pNext;
        } while (pCurr != NULL);
    }
    return NiRoborio_Status_Success;
}
              ...
              uint32_t* irqAssert, NiFpga_Bool* continueWaiting)
{
    ...
    do
    {
         ...
         *
         */
        status = NiFpga_WaitOnIrqs(roborio_session, irqContext, 1 << irqNumber,
    ...
    {
        RoboRio_ReturnIfNotSuccess(status,
...
void Irq_Acknowledge(uint32_t irqAssert)
{
     ...
     */
    status = NiFpga_AcknowledgeIrqs(roborio_session, irqAssert);
